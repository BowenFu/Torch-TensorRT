...
from .._specs import CuTeDSLSpec, CuTileSpec, FromMethodSelf, TritonSpec
...
class CustomPluginSpec:
    ...
    1. **Creation** — ``custom_plugin()`` constructs a ``CustomPluginSpec``
       by splitting kwargs into weights/self_attrs/attrs, hashing the kernel specs
    ...
    Attributes:
        ...
        self_attrs:  ``FromMethodSelf`` values declared as kwargs in
                     ``custom_plugin(w=tta.self_attr("scale"))``.  These flow through
                     region IO at trace time (like function-signature self_attr).
    """
    ...
    # FromMethodSelf values declared as kwargs in custom_plugin(w=tta.self_attr("scale")).
    # These flow through region IO at trace time (like function-signature self_attr).
    self_attrs: Dict[str, "FromMethodSelf"] = field(default_factory=dict)
    ...
    def register_dynamo_plugin(
        ...
        self.register_qdp(op_name, len(tensor_input_names))
    ...
    def auto_register_torch_op(self, op_name: str) -> None:
        ...
        def _eager_body(*args: torch.Tensor) -> Any:
            ...
            return outs[0] if _num_outputs == 1 else tuple(outs)
        ...
        sig = inspect.Signature(sig_params, return_annotation=torch.Tensor)
...
def _infer_num_outputs(meta_impl: Optional[Callable[..., Any]]) -> int:
    ...
    Returns 1 if ``meta_impl`` is ``None``, if any dispatch mode is active, or if
    ``meta_impl`` raises with 1-D dummies (e.g. shape-sensitive meta_impls).

    Args:
        meta_impl: Optional meta function to probe.  May be ``None``.
...
def custom_plugin(
    ...
    **kwargs: Any,
    ...
    Args:
        ...
        meta_impl: Optional.  PyTorch meta function used by QDP for shape/dtype
        ...
        **kwargs:  Plugin-level keyword arguments, split by type at creation time:
                   ...
                   * ``FromMethodSelf`` values → **self_attrs**: resolved from
                     ``self`` at trace time and flow through region IO.
    ...
    if meta_impl is not None and not callable(meta_impl):
        raise TypeError(
            f"custom_plugin: meta_impl must be callable or None, "
            f"got {type(meta_impl).__name__!r}"
        )

    # Split kwargs by value type:
    #   torch.Tensor       → weights (TRT constant layers injected at lowering)
    #   FromMethodSelf     → self_attrs (resolved from self at trace time, flow via region IO)
    #   everything else    → attrs (scalar compile-time constants)
    weights: Dict[str, torch.Tensor] = {}
    self_attrs: Dict[str, FromMethodSelf] = {}
    attrs: Dict[str, Any] = {}
    for k, v in kwargs.items():
        if isinstance(v, torch.Tensor):
            weights[k] = v
        elif isinstance(v, FromMethodSelf):
            self_attrs[k] = v
        else:
            attrs[k] = v
    ...
    return CustomPluginSpec(
        ...
        num_outputs=num_outputs, attrs=attrs, weights=weights, self_attrs=self_attrs,
    )
...
def _build_desc_fn(
    ...
    if descriptor.meta_impl is not None:
        _desc = _build_meta_impl_desc_fn(descriptor.meta_impl, num_outputs, num_inputs)
    else:
        _desc = _build_identity_desc_fn(num_outputs)
...
def _format_str_for_tactic(
    entry: TacticEntry,
    specs: List[KernelSpec],
    num_inputs: int,
    num_outputs: int,
) -> str:
    """Return a per-tensor ``AutoTuneCombination`` format string for a single tactic entry.

    Builds a comma-separated format string covering all inputs then all outputs,
    using the ``input_formats`` / ``output_formats`` lists on the kernel spec
    selected by ``entry.spec_idx``.  Falls back to ``"LINEAR"`` for any I/O
    position whose format is absent, out of range, or not in the known token map.
    ...
    Returns:
        A comma-separated format string such as ``"LINEAR, CHW32, LINEAR"``.
    """
    spec = specs[entry.spec_idx]
    input_fmts = getattr(spec, "input_formats", None) or []
    output_fmts = getattr(spec, "output_formats", None) or []
    parts: List[str] = []
    for i in range(num_inputs):
        fmt = input_fmts[i] if i < len(input_fmts) else None
        parts.append(_TRT_FORMAT_TOKEN.get(fmt, "LINEAR"))
    for j in range(num_outputs):
        fmt = output_fmts[j] if j < len(output_fmts) else None
        parts.append(_TRT_FORMAT_TOKEN.get(fmt, "LINEAR"))
...
def _build_autotune_fn(
    ...
    * ``format_str`` — comma-separated per-I/O memory formats,
      e.g. ``"LINEAR, CHW32, LINEAR"`` (inputs then outputs)
    * ``tactic_ids`` — list of 1-based integer tactic IDs (0 is reserved by TRT)
    ...
    # Pre-compute per-tensor format string per tactic so the closure doesn't
    # need to re-evaluate on every call to _autotune_fn.
    _tactic_formats = [
        _format_str_for_tactic(entry, descriptor.specs, num_inputs, num_outputs)
