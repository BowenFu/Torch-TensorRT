...
# NOTE: collect_allowed_formats_for_io returns the *union* of formats across all
# specs for a given I/O position.  It is not called by _build_autotune_fn, which
# uses _format_str_for_tactic to derive a per-tactic, per-tensor format string
# directly from each spec's input_formats/output_formats.  This function is
# available for external callers (e.g. converters) that need to enumerate all
# formats a plugin can accept for a given I/O slot.
...
def make_meta_tensor_from_td(td: Any) -> torch.Tensor:
    """Create a meta tensor with the shape/dtype of a QDP TensorDesc.

    Fake/symbolic shape dimensions are resolved to their minimum valid value (1).
    """
    shape = tuple(_shape_expr_to_ints(td.shape_expr))
    torch_dtype = td_dtype_to_torch(td.dtype)
    return torch.empty(shape, dtype=torch_dtype, device="meta")


def make_meta_tensor_from_td_symbolic(td: Any) -> torch.Tensor:
    """Create a meta tensor with symbolic dimensions from a TensorDesc.

    Used during the first-call meta_impl pass where distinct fake/symbolic
    dimensions must be assigned distinct placeholder sizes so that shape
    propagation through the user's forward() is correct.
    """
...
def _output_shape_to_shape_expr(shape: Tuple[Any, ...], symbolic: bool = False) -> List[Any]:
    ...
    Args:
        shape:    Tuple of dimension values (int or torch.SymInt).
        symbolic: If True, preserve torch.SymInt objects as-is (no ``int()``
                  coercion); used during first-call meta_impl to keep symbolic
                  information alive for TRT shape inference.
    """
    if not _TRT_AVAILABLE:
        return list(shape)
    if symbolic:
        return [trtp.SymInt32(d) if isinstance(d, int) else d for d in shape]
    return [trtp.SymInt32(int(d)) for d in shape]


def make_td_from_meta_using_template(template_td: Any, t: torch.Tensor) -> Any:
    """Create an output TensorDesc from a meta tensor using template.like().

    Uses ``template_td.like()`` then mutates shape/dtype rather than calling
    the TensorDesc constructor directly, which segfaults on some bindings
    versions when shape_expr is provided as a constructor argument.

    The shape is only overridden when the output shape differs from the
    template shape, avoiding unnecessary mutations.
    """
    out_td = template_td.like()
    template_shape = tuple(_shape_expr_to_ints(template_td.shape_expr))
    output_shape = tuple(int(d) for d in t.shape)
    if output_shape != template_shape:
        if _TRT_AVAILABLE:
            out_td.shape_expr = [trtp.SymInt32(d) for d in output_shape]
        else:
            out_td.shape_expr = list(output_shape)
    out_td.dtype = torch_dtype_to_trt(t.dtype)
    return out_td


def make_td_from_meta_using_template_symbolic(template_td: Any, t: torch.Tensor) -> Any:
    """Like make_td_from_meta_using_template but preserves symbolic output shape.

    Used during the first-call meta_impl pass to carry torch.SymInt dimensions
    through to TRT's shape inference without coercing them to concrete ints.
    """
    out_td = template_td.like()
    out_td.shape_expr = _output_shape_to_shape_expr(tuple(t.shape), symbolic=True)
    out_td.dtype = torch_dtype_to_trt(t.dtype)
    return out_td
