...
_CUDNN_TEST_FILES = [
    os.path.join(os.path.dirname(__file__), "integration", "test_plugin_e2e.py"),
    os.path.join(os.path.dirname(__file__), "integration", "test_cudnn_plugin_e2e.py"),
]
...
def pytest_sessionfinish(session, exitstatus):
    ...
    if pre_bw_files:
        ...
        cmd = (
            [sys.executable, "-m", "pytest", "-v", "--tb=short", "-n", "12",
             "-m", "requires_pre_bw"]
            + pre_bw_files
        )
