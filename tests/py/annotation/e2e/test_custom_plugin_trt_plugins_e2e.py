...
def _engine_has_pluginv3_layer(engine, op_name):
    ...
    *op_name* is 'namespace::name' (e.g. 'torchtrt_e2e_triton::swiglu').
    ...
    Two conditions:
      ...
      2. A layer whose name starts with '/<name>' appears in the engine.
         TRT's Myelin optimizer may append suffixes such as _myl0_0, so
         prefix matching is used rather than exact string equality.
    """
    ...
    prefix = f"/{name}"
    return any(ln == prefix or ln.startswith(prefix + "_") for ln in layer_names)
